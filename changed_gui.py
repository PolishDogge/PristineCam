...
try:
    ...
    from PyQt6.QtGui import QColor, QFont, QIcon, QImage, QIntValidator, QPainter, QPixmap
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self._s_battery = _StatRow("Battery",    "—")
        for w in (self._s_state, self._s_fps, self._s_lat, self._s_res, self._s_battery):
    ...
    def _poll_battery(self) -> None:
        """Fetch /status from the phone on a daemon thread (non-blocking)."""
        url = self._status_url
        if not url:
            return
        worker = self._worker  # capture ref — worker may be replaced
        if worker is None:
            return
    ...
    def _on_battery_updated(self, level: int, charging: bool) -> None:
        if level < 0:
            self._s_battery.set_text("—")
            return
        symbol = " (Charging)" if charging else ""
        ...
        if level >= 50:
            ...
            color = "#ef4444"
        self._s_battery.set_text(f"{level}%{symbol}", color=color)
    ...
    def _set_ui_connected(self, connected: bool) -> None:
        ...
        if connected:
            self._poll_battery()           # immediate first poll
